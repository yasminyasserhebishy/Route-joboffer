...
export const allAccounts = asyncHandler(async (req, res, next) => {
    ...
    //check if accounts with recoveryEmail exist
    const userAccounts = await userModel.find({ recoveryEmail }).select('userName email recoveryEmail gender DOB')
    // Check if there are any accounts
    if (!userAccounts || userAccounts.length === 0) {
        return next(new Error('No accounts found for the provided recovery email'),{cause : 404})
    }
    //Return the list of user accounts in the API response
    return res.status(200).json({ message: 'Accounts found successfully', userAccounts });
})
